(function ($) {
	...
	function updatePreview() {
		...
		if (widthUnit === '%') {
			...
			preview.style.width = Math.min(100, Math.max(20, widthValue / 12)) + '%';
		}
		...
		if (mode === 'aspect') {
			...
			const px = num(nestedValue(device, 'height_value', 620), 620);
			preview.style.height = Math.max(180, Math.min(420, px * .62)) + 'px';
		}
		...
		if (title) {
			...
			if (val('title_font_mode', 'clamp') === 'clamp') {
				title.style.fontSize =
					'clamp(' + num(val('title_clamp_min', 26), 26) + 'px,' +
					num(val('title_clamp_fluid', 3.1), 3.1) + 'vw,' +
					num(val('title_clamp_max', 52), 52) + 'px)';
			} else {
				const key = device === 'mobile' ? 'title_mobile' : device === 'tablet' ? 'title_tablet' : 'title_desktop';
				title.style.fontSize = num(val(key, 42), 42) + 'px';
			}
		}
		...
		if (cta) {
			...
			cta.style.color = val('cta_text_color', '#ffffff');
			cta.style.backgroundColor = val('cta_bg_color', '#2f6f3e');
			...
			cta.style.fontSize = num(val('cta_font_size', 16), 16) + 'px';
